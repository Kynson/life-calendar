...
<script lang="ts">
  ...
  const dispatchEvent = createEventDispatcher<ChangeEvent>();
...
<label class="flex flex-space-between">
  ...
  <div class="input-container">
    <input bind:this={input} on:input={handleInput} {placeholder} {type} {name} {id}>
    {#if type === 'file'}
      <div>
        <button type="button" on:click={handleChooseFileClick}>Choose file</button>
        <span>{selectedFileName || 'No file chosen'}</span>
      </div>
    {/if}
    {#if type === 'color'}
      <div style="background-color: {value || 'rgb(0, 0, 0)'}"></div>
    {/if}
  </div>
</label>
